...
public func makePixelSet(width: Int, height: Int) -> ([Pixel], Int, Int) {
    ...
    for i in 0..<width {
        for j in 0..<height {
            ...
            {
                debugPrint("\(lower_left_corner) + \(u) * \(horizontal) + \(v) = \(lower_left_corner + u * horizontal)")
                debugPrint("\(lower_left_corner) + \(u) * \(horizontal) + \(v) * \(vertical) = \(lower_left_corner + u * horizontal + v * vertical)")
            }
            let col = color(r: r)
